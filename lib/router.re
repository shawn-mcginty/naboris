...
let isParamNode = (node) => (String.get(node, 0) == ':');
...
let rec compilePathMatcher = (pathList, regexStr, args) => {
	...
	| [headNode, ...rest] => compilePathMatcher(rest, regexStr ++ getNodeRegexStr(headNode), addArgIfNeeded(headNode, args))
	}
};

let getPathRegex = (path) => {
	/* throw away first item, it's just an empty string */
	let treeBranch = switch(String.split_on_char('/', path)) {
...
let addNewMethod = (route, methods: list((Method.t, requestHandler))) => {
	...
	| Some(_) => raise(DuplicateRoute("Duplicate route handler defined in server options for path " ++ route.path));
...
let addMethodHandler = (route: route, handlers: list(handler)) => {
	List.map((handler: handler) => switch (handler.path == route.path) {
	| true => addMethodToHandler(route, handler);
	| false => handler;
	}, handlers);
};
