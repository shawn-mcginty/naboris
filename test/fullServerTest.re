...
let timeout = (time) => Lwt_unix.sleep(time) >>= () => Lwt.return((TestResult.TestTimeout, time));
...
let reportFailed = (title, (msg, line, col)) => print_string(" ❌ " ++ title
	++ " " ++ msg ++ " " ++ string_of_int(line) ++ ":" ++ string_of_int(col) ++ "\n");

let rec startTests = (~total=0, ~success=0, ~failed=0, tests) => {
	switch (tests) {
	| [] => {
		print_string("\n\nDone! " ++ string_of_int(success) ++ " passing    " ++ string_of_int(failed) ++ " failing.\n\n");
		if (failed > 0) {
	...
	routes: [{
		...
		requestHandler: (req, res) => {
			Naboris.Res.status(200, res)
				|> Naboris.Res.html(req, "<!doctype html><html><body>You made it.</body></html>");
		}
	}, {
		method: GET,
		...
		requestHandler: (req, res) => {
			Naboris.Res.status(200, res)
				|> Naboris.Res.html(req, "<!doctype html><html><body>You want some user id and item</body></html>");
		}
	}]
};
